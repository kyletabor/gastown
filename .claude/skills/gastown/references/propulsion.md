...
## Step Transitions: Before and After
...
1. **Waiting for confirmation when work is hooked**: The hook IS your assignment. Execute immediately.
2. **Announcing yourself before starting**: Just start. The system knows you're active when you close steps.
3. **Asking what to do when hooked work exists**: Run `gt hook` and `bd mol current` to discover your assignment.
4. **Using multiple commands when `--continue` works**: Preserve momentum with single-command transitions.
